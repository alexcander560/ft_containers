...
int	test(int size, string test, bool random = false)
{
	...
	std::pair<std::map<int, string>::iterator, int> pair_orig;
	ft::map<int, string> my;
	ft::pair<ft::map<int, string>::iterator, int> pair_my;
	...
	{
		if (random)
			pair_orig = orig.insert(std::pair<int, string>(base[i], to_string(base[i]) + "_"));
		else
			pair_orig = orig.insert(std::pair<int, string>(i, to_string(i) + "_"));

		temp_orig += to_string(pair_orig.first->first) + pair_orig.first->second + "(" + to_string(pair_orig.second) + ")";
	}
	...
	{
		if (random)
			pair_my = my.insert(ft::pair<int, string>(base[i], to_string(base[i]) + "_"));
		else
			pair_my = my.insert(ft::pair<int, string>(i, to_string(i) + "_"));

		temp_my += to_string(pair_my.first->first) + pair_my.first->second + "(" + to_string(pair_my.second) + ")";
	}
